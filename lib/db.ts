...
function ensureTodoMigrations(db: Database.Database): void {
  ...
  db.exec('CREATE INDEX IF NOT EXISTS idx_todos_recurring ON todos(user_id, is_recurring, due_date);')
}
  ...
  create(dto: CreateTodoDto): Todo {
    ...
    const createTodo = db.transaction((payload: CreateTodoDto) => {
      ...
      const row = stmt.get(
        ...
        payload.is_recurring ? (payload.recurrence_pattern ?? null) : null,
      ) as TodoRow
    ...
    const transaction = db.transaction((todoId: number, patch: UpdateTodoDto, scopedUserId: number) => {
      ...
      if (transitionedToCompleted && updatedTodo.is_recurring) {
        ...
        nextInstance = this.create({
          ...
          tag_ids: updatedTodo.tags.map((tag) => tag.id),
        })
      }
